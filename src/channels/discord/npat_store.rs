//! What Name Place Animal Thing keeps in `.runtime/npat.db`: every round and its
//! letter, each player's last submission, the verdict on every answer, the
//! game scores and places (with the house points each place is owed and what
//! the ledger actually credited), challenges, and the verdict cache.
//!
...
//! the first two. Each move is one `UPDATE … WHERE status = …`, so a stop that
//! lands while the model is judging wins and nothing is paid.
...
use super::npat_judge::{Judged, Prizes, Scored, Verdict};
...
const ROUND_COLS: &str =
    "id, channel_id, message_id, results_message_id, letter, started_at, ends_at, status, letter_only, pays, paid_out, judged_at";
...
fn round_row(r: &rusqlite::Row) -> rusqlite::Result<Round> {
    Ok(Round {
        ...
        results_message: r.get::<_, Option<i64>>(3)?.map(|m| m as u64),
        letter: r.get::<_, String>(4)?.chars().next().unwrap_or('A'),
        started_at: r.get(5)?,
        ends_at: r.get(6)?,
        status: Status::from_key(&r.get::<_, String>(7)?),
        letter_only: r.get::<_, i64>(8)? != 0,
        pays: r.get::<_, i64>(9)? != 0,
        paid_out: r.get::<_, i64>(10)? != 0,
        judged_at: r.get(11)?,
    })
}
...
/// Finished rounds whose points or results card a restart may have left undone.
pub fn unfinished(conn: &Connection) -> Vec<Round> {
    let sql = format!(
        "SELECT {} FROM rounds WHERE status = 'done' AND ((pays = 1 AND paid_out = 0) OR results_message_id IS NULL) ORDER BY id",
        ROUND_COLS
    );
    let Ok(mut stmt) = conn.prepare(&sql) else {
        return Vec::new();
    };
    stmt.query_map([], round_row).map(|rows| rows.flatten().collect()).unwrap_or_default()
}
...
pub fn start_round(conn: &Connection, channel: u64, letter: char, now: i64, secs: i64) -> rusqlite::Result<Round> {
    conn.execute(
        "INSERT INTO rounds (channel_id, letter, started_at, ends_at) VALUES (?1, ?2, ?3, ?4)",
        params![channel as i64, letter.to_string(), now, now + secs],
    )?;
    let id = conn.last_insert_rowid();
    get_round(conn, id).ok_or(rusqlite::Error::QueryReturnedNoRows)
}
...
pub fn finish_judging(conn: &Connection, id: i64, letter_only: bool, pays: bool, now: i64) -> rusqlite::Result<bool> {
    conn.execute(
        "UPDATE rounds SET status = 'done', letter_only = ?2, pays = ?3, judged_at = ?4, day = ?5 WHERE id = ?1 AND status = 'judging'",
        params![id, letter_only as i64, pays as i64, now, super::points::ist_day(now)],
    )
    .map(|n| n > 0)
}

pub fn mark_paid_out(conn: &Connection, id: i64) -> rusqlite::Result<()> {
    conn.execute("UPDATE rounds SET paid_out = 1 WHERE id = ?1", params![id]).map(|_| ())
}

/// Stops the live round, if there is one: no judging, no points.
pub fn stop_live(conn: &Connection, by: u64) -> rusqlite::Result<Option<Round>> {
    let Some(round) = live_round(conn) else {
        return Ok(None);
    };
    let n = conn.execute(
    ...
    Ok((n > 0).then(|| Round { status: Status::Stopped, ..round }))
}
...
/// A score row: someone's game score and place, the house points that place is
/// owed, and what the ledger credited.
...
/// Writes the scores and places, keeping what was credited and the fix
/// counters. `prizes` is `None` for a round that pays nothing.
pub fn save_scores(conn: &mut Connection, round_id: i64, scored: &[Scored], prizes: Option<Prizes>) -> rusqlite::Result<()> {
    let tx = conn.transaction()?;
    for s in scored {
        let owed = prizes.map(|p| p.for_place(s.place)).unwrap_or(0);
        tx.execute(
            "INSERT INTO scores (round_id, user_id, score, unique_count, place, owed) VALUES (?1, ?2, ?3, ?4, ?5, ?6)
             ON CONFLICT(round_id, user_id) DO UPDATE SET score = excluded.score, unique_count = excluded.unique_count,
                 place = excluded.place, owed = excluded.owed",
            params![round_id, s.user as i64, s.score, s.unique, s.place as i64, owed],
        )?;
    }
    tx.commit()
}
...
pub fn scores(conn: &Connection, round_id: i64) -> HashMap<u64, ScoreRow> {
    let Ok(mut stmt) = conn.prepare("SELECT user_id, score, unique_count, place, owed, credited, fixes FROM scores WHERE round_id = ?1") else {
        return HashMap::new();
    };
    stmt.query_map(params![round_id], |r| {
        Ok((
            r.get::<_, i64>(0)? as u64,
            ScoreRow { score: r.get(1)?, unique: r.get(2)?, place: r.get::<_, i64>(3)? as u8, owed: r.get(4)?, credited: r.get(5)?, fixes: r.get(6)? },
        ))
    })
    .map(|rows| rows.flatten().collect())
    .unwrap_or_default()
...
pub fn add_credit(conn: &Connection, round_id: i64, user: u64, granted: i64) -> rusqlite::Result<()> {
    conn.execute(
        "UPDATE scores SET credited = credited + ?3 WHERE round_id = ?1 AND user_id = ?2",
        params![round_id, user as i64, granted],
    )
    .map(|_| ())
}

/// The next fix number for someone's round: 1, 2, 3…
pub fn next_fix(conn: &Connection, round_id: i64, user: u64) -> rusqlite::Result<i64> {
    conn.execute("UPDATE scores SET fixes = fixes + 1 WHERE round_id = ?1 AND user_id = ?2", params![round_id, user as i64])?;
    conn.query_row("SELECT fixes FROM scores WHERE round_id = ?1 AND user_id = ?2", params![round_id, user as i64], |r| r.get(0))
}
...
/// Every 1st and 2nd place on an India day, over rounds that paid: (user,
/// place, when the round was judged), oldest first.
pub fn places_on(conn: &Connection, day: &str) -> Vec<(u64, u8, i64)> {
    let Ok(mut stmt) = conn.prepare(
        "SELECT s.user_id, s.place, r.judged_at FROM scores s JOIN rounds r ON r.id = s.round_id
         WHERE r.day = ?1 AND r.status = 'done' AND r.pays = 1 AND s.place IN (1, 2) ORDER BY r.judged_at, r.id, s.place",
    ) else {
        return Vec::new();
    };
...
pub(crate) mod tests {
    ...
    fn a_round_takes_the_last_answer_in_time_and_moves_through_its_states() {
        let mut conn = memory();
        let round = start_round(&conn, 7, 'P', NOON, 45).unwrap();
        ...
        assert!(finish_judging(&conn, round.id, false, true, NOON + 60).unwrap());
        let done = get_round(&conn, round.id).unwrap();
        assert_eq!((done.status, done.pays, done.paid_out), (Status::Done, true, false));
        assert!(live_round(&conn).is_none());
        assert_eq!(unfinished(&conn).len(), 1);
        mark_paid_out(&conn, round.id).unwrap();
        set_results_message(&conn, round.id, 55).unwrap();
    ...
    fn a_stop_beats_judging() {
        let conn = memory();
        let round = start_round(&conn, 7, 'M', NOON, 45).unwrap();
        assert!(begin_judging(&conn, round.id).unwrap());
        assert_eq!(stop_live(&conn, 9).unwrap().map(|r| r.status), Some(Status::Stopped));
        assert!(!finish_judging(&conn, round.id, false, true, NOON + 60).unwrap(), "stopped: nothing to finish");
        assert_eq!(status_of(&conn, round.id), Some(Status::Stopped));
        assert!(stop_live(&conn, 9).unwrap().is_none());
        assert!(!begin_judging(&conn, round.id).unwrap());
        let next = start_round(&conn, 7, 'K', NOON + 100, 45).unwrap();
        assert_eq!(recent_letters(&conn, 5), vec!['K', 'M']);
        assert_eq!(live_round(&conn).map(|r| r.id), Some(next.id));
    }

    #[test]
    fn a_review_flips_one_answer_reranks_the_round_and_counts_fixes() {
        let mut conn = memory();
        let p = Points { unique: 10, shared: 5 };
        let prizes = Some(Prizes { first: 2, second: 1 });
        let round = start_round(&conn, 7, 'P', NOON, 45).unwrap();
        submit(&conn, round.id, 1, "gryffindor", &answers(["Priya", "Pondicherry", "", ""]), NOON + 1).unwrap();
        submit(&conn, round.id, 2, "gryffindor", &answers(["", "Puducherry", "Panda", ""]), NOON + 2).unwrap();
        submit(&conn, round.id, 3, "gryffindor", &answers(["", "Patna", "", ""]), NOON + 3).unwrap();
        begin_judging(&conn, round.id).unwrap();
        let v = |valid: bool, c: &str| judge::Verdict { valid, canonical: c.into() };
        save_verdicts(
            &mut conn,
            round.id,
            &[
                (1, 0, "Priya".into(), v(true, "Priya")),
                (1, 1, "Pondicherry".into(), v(false, "")),
                (2, 1, "Puducherry".into(), v(true, "Puducherry")),
                (2, 2, "Panda".into(), v(true, "Panda")),
                (3, 1, "Patna".into(), v(true, "Patna")),
            ],
        )
        .unwrap();
        let before = judge::score(&judged(&conn, round.id), p);
        save_scores(&mut conn, round.id, &before, prizes).unwrap();
        add_credit(&conn, round.id, 2, 2).unwrap();
        add_credit(&conn, round.id, 1, 1).unwrap();
        let old = scores(&conn, round.id);
        assert_eq!((old[&2].score, old[&2].place, old[&2].owed), (20, 1, 2));
        assert_eq!((old[&1].score, old[&1].place, old[&1].owed), (10, 2, 1), "10 each for 1 and 3: 1 answered first");
        assert_eq!((old[&3].place, old[&3].owed), (0, 0));

        // The model missed that Pondicherry and Puducherry are the same place, and a mod says it's valid.
        add_challenges(&conn, round.id, 1, &[1, 1, 9], NOON + 70).unwrap();
        assert_eq!(challenges(&conn, round.id), vec![(1, 1, false)]);
        let flipped = toggle_verdict(&conn, round.id, 1, 1, 42, NOON + 80).unwrap().unwrap();
        assert_eq!(flipped, v(true, "Pondicherry"));
        conn.execute("UPDATE round_verdicts SET canonical = 'Puducherry' WHERE round_id = ?1 AND user_id = 1 AND category = 1", params![round.id]).unwrap();
        assert_eq!(challenges(&conn, round.id), vec![(1, 1, true)], "the review resolves the challenge");
        let after = judge::score(&judged(&conn, round.id), p);
        save_scores(&mut conn, round.id, &after, prizes).unwrap();
        let new = scores(&conn, round.id);
        assert_eq!((new[&1].score, new[&1].place, new[&1].owed), (15, 1, 2), "Priya unique + a shared place, and they finished answering first");
        assert_eq!((new[&2].score, new[&2].place, new[&2].owed), (15, 2, 1), "Panda unique + the now shared place");
        assert_eq!(judge::adjustment(old[&1].owed, new[&1].owed, old[&1].credited), 1);
        assert_eq!(judge::adjustment(old[&2].owed, new[&2].owed, old[&2].credited), -1);
        assert_eq!(after.iter().find(|s| s.user == 2).unwrap().marks[1], Mark::Shared);
        assert_eq!(new[&1].credited, 1, "re-ranking keeps what was credited");
        assert_eq!(next_fix(&conn, round.id, 1).unwrap(), 1);
        assert_eq!(next_fix(&conn, round.id, 1).unwrap(), 2);
        assert!(toggle_verdict(&conn, round.id, 1, 3, 42, NOON).unwrap().is_none(), "a blank has nothing to flip");

        finish_judging(&conn, round.id, false, true, NOON + 60).unwrap();
        let places = places_on(&conn, "2026-09-14");
        assert_eq!(places.len(), 2);
        assert!(places.iter().all(|(_, _, at)| *at == NOON + 60));
        conn.execute("UPDATE rounds SET pays = 0", []).unwrap();
        assert!(places_on(&conn, "2026-09-14").is_empty(), "rounds that didn't pay don't count");
    }
}
