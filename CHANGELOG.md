...
- Freaky (18+) genre - only offered in an age-restricted quiz channel with VIZIER_QUIZ_FREAKY on, never in the mix
...
- *(discord)* Rename /letters to /nochitthi
...
- *(discord)* /letters opts a member out of receiving letters
...
- *(discord)* Admin-only /stop and /resume
- *(discord)* Admin-only mode toggle
- *(discord)* Register /help, make it ephemeral, rewrite for MLCI
- *(discord)* Ignore direct messages entirely
